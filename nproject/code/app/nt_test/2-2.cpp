...
#include "dppclient.h"
//读数量
int TestHDARead1(CGAP_HSDASrvClient &Client)
{
  CSgDataSet DatSet;
	...
	ReadReq.SQLBuf.Format("SELECT COUNT(*) FROM MODEL_D_BUSBAR_SECTION");
...
int TestHDARead(CGAP_HSDASrvClient &Client)
{
  CSgDataSet DatSet;
	...
	ReadReq.SchemaHandler = 1;
	ReadRes.pRetDat = &DatSet;
	ReadReq.SQLBuf.Format("select * from MODEL_D_BUSBAR_SECTION WHERE BASE_VOLTAGE=22");
	if (Client.HSDA_ReadRecorder(ReadReq, ReadRes) < 0)
	{
		return false;
	}
